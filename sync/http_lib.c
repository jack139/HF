...
static char *http_user_agent="F8-SYNC/1.3 (an Kam sync client from Kam@Cloud)";
  ...
  if (conn_nonb(s, &server, sizeof(server), 5, 0) < 0) ret=ERRCONN;
    ...
    if (webpy_session[0])
      sprintf(query_header[threadnum],
          ...
          "%s"
          "Cookie: kam_session=%s\015\012\015\012",
  	      command, url,
...
http_retcode http_get(char *filename, int *plength, char *typebuf, int threadnum)
{
  ...
  if (ret==200) {
    while (1) {
      ...
      sscanf(http_header[threadnum],"set-cookie: kam_session=%[^;];",webpy_session);
